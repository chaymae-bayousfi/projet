...
def get_api_key_safely(key_name):
    """Safely get API key from secrets or environment without raising exceptions"""
    try:
    ...
    if st.button("Submit Answer"):
        if not user_answer.strip():
            ...
            if "answer" in exercise and exercise["answer"]:
                ...
                # Get personalized feedback using Mistral
                mistral_api_key = get_api_key_safely("MISTRAL_API_KEY")
                if mistral_api_key:
                    ...
                    st.info("💡 Add your Mistral API key in the sidebar for AI-powered feedback!")
            else:
...
def visualization_ui():
    ...
    if st.session_state.algorithm_steps:
        ...
        # Get detailed explanation from Mistral if API key is provided
        mistral_api_key = get_api_key_safely("MISTRAL_API_KEY")
        if mistral_api_key:
            ...
            st.info("💡 Add your Mistral API key in the sidebar for AI-powered step explanations!")
        ...
        with st.expander("Need a hint?"):
            ...
            if hint_question:
                ...
                if mistral_api_key:
                    ...
                    st.warning("Please add your Mistral API key in the sidebar to get AI hints!")
    ...
    with col1:
        ...
        if len(st.session_state.graph.nodes()) > 1:
            ...
            st.info("Add more nodes to create edges!")
    ...
    with col2:
        ...
        if len(st.session_state.graph.nodes()) > 1:
            ...
            # Check path between start node and last node
            nodes_list = list(st.session_state.graph.nodes())
            if len(nodes_list) > 1:
                last_node = nodes_list[-1]
                if nx.has_path(st.session_state.graph, st.session_state.start_node, last_node):
                    st.success(f"Path exists from node {st.session_state.start_node} to node {last_node}")
                else:
                    st.warning(f"No path from node {st.session_state.start_node} to node {last_node}")
        else:
            st.info("Graph needs at least 2 nodes to check connectivity!")